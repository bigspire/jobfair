...
class Paging
{
	...
	function print_link_frontend($language_url='')
	{
	//generate template
...
//vidhya
		...
		if ($this->p["count"]>$this->p["baris"]) {
		   ...
		   $page_name = $_GET['pagen'];
			
			// print prev
			
			if ($this->page>1)
			$print_link .= "<li><a  href='".webroot.$language_url.$page_name."/?".($_POST?'':$_SERVER["QUERY_STRING"])."&page$this->paging_no=".($this->page-1)."".$this->search_url."'>".$this->prev."</a></li>";
			...
			{
				for ($i=$this->p["bawah"];$i<=$this->page-1;$i++)
					$print_link .="<li><a  href='".webroot.$language_url.$page_name."/?".($_POST?'':$_SERVER["QUERY_STRING"])."&page$this->paging_no=$i".$this->search_url."' title='Start'>".$this->number($i,$this->number)."</a></li>";
			}
			// print active
			if ($this->p["total_page"]>1)
    			
				$print_link .= "<li class=\"selected\"><a class='selectedPage'>".$this->number($this->page,$this->number)."<a/></li>";
	
	

			// print next numbers
			for ($i=$this->page+1;$i<=$this->p["atas"];$i++)
			$print_link .= "<li><a  href='".webroot.$language_url.$page_name."/?".($_POST?'':$_SERVER["QUERY_STRING"])."&page$this->paging_no=$i".$this->search_url."'>".$this->number($i,$this->number)."</a></li>";

			// print next
			if ($this->page<$this->p["total_page"])
			$print_link .= "<li><a  href='".webroot.$language_url.$page_name."/?".($_POST?'':$_SERVER["QUERY_STRING"])."&page$this->paging_no=".($this->page+1)."".$this->search_url."'>".$this->next."</a></li>";
