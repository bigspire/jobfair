<?php
/* 
   Purpose : To list and search job fair photos.
   Created : Nikitasa
   Date : 24-01-2017 
*/
session_start();
...
$query = "CALL list_jobfair_photo('".$keyword."','".$status."','0','0','','','".$_GET['action']."')";
try{
	...
	if($count == 0){
		if($keyword){
			$alert_msg = 'No job fair photos "' .$keyword. '" is found in our database';
		}else{
			$alert_msg = 'No job fair photos found in our database';
		}
	}
...
// fetch all records
$query = "CALL list_jobfair_photo('".$keyword."','".$status."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
...
try{
	...
	while($obj = $mysql->display_result($result))
	{
		...
		$data[$i]['jobfair_date'] = $fun->format_date($obj['jobfair_date']);
		$i++;
 		$pno[]=$paging->print_no();
	...
	if($_GET['current_status'] == 'deleted' || $_GET['current_status'] == 'created' || $_GET['current_status'] == 'updated' ){
  		$success_msg = 'Job Fair photos ' . $_GET['current_status'] . ' successfully';
...
// assign page title
$smarty->assign('page_title' , 'Job Fair Photos'); 
// display smarty file
$smarty->display('jobfair_photos.tpl');
?>