<?php
/* 
   Purpose : To list and search job fair logo.
   Created : Nikitasa
   Date : 19-11-2016 
*/
session_start();
...
$query = "CALL list_jobfair_logos('".$keyword."','".$status."','0','0','','','".$_GET['action']."')";
try{
	...
	if($count == 0){
		if($keyword){
			$alert_msg = 'No job fair logo "' .$keyword. '" is found in our database';
		}else{
			$alert_msg = 'No job fair logo found in our database';
		}
	}
...
$sort_fields = array('1' => 'company_name','logo','title','status','created_date','modified_date');
$org_fields = array('1' => 'company_name','logo', 'title','status','created_date','modified_date');
...
// fetch all records
$query = "CALL list_jobfair_logos('".$keyword."','".$status."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
...
try{
	...
	if($_GET['current_status'] == 'deleted' || $_GET['current_status'] == 'created' || $_GET['current_status'] == 'updated' ){
  		$success_msg = 'Job Fair Logo ' . $_GET['current_status'] . ' successfully';
...
// assign page title
$smarty->assign('page_title' , 'Job Fair Logo'); 
// display smarty file
$smarty->display('jobfair_logos.tpl');
?>