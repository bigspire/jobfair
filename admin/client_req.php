<?php
/* 
   Purpose : To list and search client req.
   Created : Nikitasa
   Date : 25-11-2016 
*/
session_start(); 
...
$query = "CALL list_client_req('".$keyword."','".$status."','".$from_date."','".$to_date."','0','0','','','".$_GET['action']."')";
try{
	...
	if($count == 0){
		if($keyword){
			$alert_msg = 'No client requirement "' .$keyword. '" is found in our database';
		}else{
			$alert_msg = 'No client requirement found in our database';
		}
	}
...
$sort_fields = array('1' => 'company_name','position','drive_date','work_loc','no_vacancy','status','created_date','modified_date','start_date','end_date');
$org_fields = array('1' => 'company_name','position','drive_date', 'work_loc','no_vacancy','status','created_date','modified_date','start_date','end_date');
...
// fetch all records
$query = "CALL list_client_req('".$keyword."','".$status."','".$from_date."','".$to_date."','$start','$limit','".$field."','".$order."','".$_GET['action']."')";
...
try{
	...
	if($_GET['current_status'] == 'deleted' || $_GET['current_status'] == 'created' || $_GET['current_status'] == 'updated' ){
  		$success_msg = 'Client Requirement  ' . $_GET['current_status'] . ' successfully';
...
if($_GET['action'] == 'export'){ 
	...
	$excelObj->printCell($dataexcl, $i,$col = array('A','B','C','D','E','F'), $field = array('full_name','email_id','mobile_no1','course_name','specialization','created_date'),'req_registrations_'.date('dmY'));
}
...
// assign page title
$smarty->assign('page_title' , 'Client Req'); 
// display smarty file
$smarty->display('client_req.tpl');
?>