...
class Paging
{
	...
	function print_link_frontend(){
		...
		if ($this->p["count"] > $this->p["baris"]){
			...
			if ($this->page>1)
			$print_link .= "<span class='fg-button ui-button'><a  href='".$page_name."?".($_POST?'':$_SERVER["QUERY_STRING"])."&page$this->paging_no=".($this->page-1)."".$this->search_url."'>".$this->prev."</a></span>";
			...
			{
				for ($i=$this->p["bawah"];$i<=$this->page-1;$i++){
					 $print_link .="<span class='fg-button ui-button'><a  href='".$page_name."?".($_POST?'':$_SERVER["QUERY_STRING"])."&page$this->paging_no=$i"."".$this->search_url."' title='Start'>".$this->number($i,$this->number)."</a></span>";
					}
			}
			// print active
			if ($this->p["total_page"]>1){
    			
				$print_link .= "<span class='fg-button ui-button SC_bold'><a>".$this->number($this->page,$this->number)."</a></span>";
	
				}

			// print next numbers
			for ($i=$this->page+1;$i<=$this->p["atas"];$i++)
			$print_link .= "<span class='fg-button ui-button'><a  href='".$page_name."?".($_POST?'':$_SERVER["QUERY_STRING"])."&page$this->paging_no=$i"."".$this->search_url."'>".$this->number($i,$this->number)."</a></span>";

			// print next
			if ($this->page<$this->p["total_page"])
			$print_link .= "<span class='fg-button ui-button'><a  href='".$page_name."?".($_POST?'':$_SERVER["QUERY_STRING"])."&page$this->paging_no=".($this->page+1)."".$this->search_url."'>".$this->next."</a></span>";
